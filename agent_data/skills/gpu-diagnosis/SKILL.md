...
1. `get_gpu_status_info()` — temperature, power, throttling, ECC, PCIe
...
2. `get_gpu_status_info()` — VRAM usage, per-process, OOM indicators
   - `VRAM > 95%` → risk of CUDA OOM
   - `orphaned allocations` → crashed process didn't release memory
3. `get_gpu_status_info()` — compute vs memory bandwidth
