...
- **排查路径**：get_system_overview → get_cluster_overview → check_kubernetes_nodes → check_network → check_processes → get_coredns_logs → check_etcd_health → check_kubernetes_pods → get_node_conditions → commit_hypotheses → 委派 network-expert → get_etcd_metrics → get_etcd_logs → get_cluster_events → record_finding × 3
