...
graph TD
    ...
    P1 --> T1["get_os_system_overview<br/>（pending → running → completed）"]
    P1 --> T2["get_os_cpu_info"]
    P1 --> T3["check_k8s_pods"]
...
| `tool_name` / `tool_args` | Tool 函数名和参数，Frontend 展示为 `get_os_cpu_info(profile="default")` |
...
```
Coordinator 尝试调用 get_argus_k8s_cluster_metrics(...)
  → ⛔ 拦截: "get_argus_k8s_cluster_metrics 是专家专用工具，请通过 task() 委派"
  → Subagent 调用同样工具 → ✅ 放行（_is_subagent=True）
```

受拦截工具：`get_argus_os_cpu_metrics/memory/disk/network/nodes/services/`
`get_argus_k8s_cluster_metrics/node/workload/pod/etcd/gpu`

**自动证据采集**：
- 每次诊断工具执行后，结果自动写入当前活跃假设的 evidence
- `task()` 委派的专家输出 → `source="expert:host-expert"` + `supports=True`
- 直接工具调用 → `source="tool:get_os_mem_5s"` + `supports=True`
...
| # | 文件 | 实现内容 | 优先级 |
|---|------|---------|--------|
| 1 | `tools/live/hosts.py` | 7 个 host 工具的 SSH/agent 远程执行逻辑（`get_os_system_overview`, `get_os_cpu_info/memory/disk/network/processes`）；补充 `get_os_net_conntrack` 和 `get_os_kernel_dmesg_err`（mock 中有但 live 中缺失） | **P0** |
| 2 | `tools/live/gpu.py` | 3 个 GPU 工具的远程执行逻辑（`get_gpu_status_info/memory/utilization`） | **P0** |
| 3 | `tools/live/argus.py`（新建） | 6 个 `query_argus_*` 监控查询工具，函数名和参数签名必须与 mock 版**完全一致** | **P0** |
| 4 | `tools/live/kubernetes.py` | `check_k8s_certificate_expiry` 从占位文本改为真实实现 | **P2** |
...
def get_argus_k8s_cluster_metrics(monitor_name: str, cluster_name: str, start_time: str = "", end_time: str = "") -> dict: ...
def get_argus_k8s_node_metrics(monitor_name: str, cluster_name: str, node_name: str, start_time: str = "", end_time: str = "") -> dict: ...
def get_argus_k8s_workload_metrics(monitor_name: str, cluster_name: str, namespace: str, workload_type: str, workload_name: str, start_time: str = "", end_time: str = "") -> dict: ...
def get_argus_k8s_pod_metrics(monitor_name: str, cluster_name: str, namespace: str, workload_name: str, pod_name: str, start_time: str = "", end_time: str = "") -> dict: ...
def get_argus_k8s_master_metrics(monitor_name: str, host_name: str = "", start_time: str = "", end_time: str = "") -> dict: ...
```
