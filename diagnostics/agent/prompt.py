...
你的诊断由诊断台账驱动（每轮注入到你的上下文）。当前阶段由系统状态机判定，**每轮「本步要求」的首行给出本阶段动作与可用工具**——按其执行即可推进诊断。
...
- **排除的假设**：验证后被排除的竞争性假设及排除原因（台账中标注「单通道证伪」的假设，须在原因后注明「证据局限：仅由单一专家通道支撑，未经跨通道复核」；系统另会在报告末尾自动补入该书面的披露段）
